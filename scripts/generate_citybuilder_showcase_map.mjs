...
const MAP_STATE_PATH = path.join(PACK_ROOT, "metadata", "generated_city_map_state.json");
const MAP_HTML_PATH = path.join(PACK_ROOT, "generated_city_map.html");
...
function renderLayerItems(entries, assetLookup, layerName) {
  return entries.map((entry) => {
    ...
    return `<img class="tile ${layerName}" src="${escapeHtml(asset.image)}" alt="${escapeHtml(title)}" title="${escapeHtml(title)}" data-asset="${escapeHtml(entry.assetId)}" style="--x:${entry.x};--y:${entry.y};--r:${entry.rotation ?? 0}deg;">`;
  }).join("\n");
}
...
function renderRailFeature(state) {
  ...
  return rail.tiles.map((tile) => (
    `<img class="tile rail-segment" src="${escapeHtml(railAsset.image)}" alt="${escapeHtml(rail.label)} (${tile.x}, ${tile.y})" title="${escapeHtml(rail.label)} (${tile.x}, ${tile.y})" data-asset="${escapeHtml(rail.assetId)}" style="--x:${tile.x};--y:${tile.y};--r:${tile.rotation ?? 0}deg;">`
  )).join("\n");
}
